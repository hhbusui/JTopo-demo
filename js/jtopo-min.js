...
function(JTopo) {
	...
	function changeColor(a, b, c, d, e) {
		//b是图片的img参数<img src=​"./​img/​server.png">
		//c 255 ​
	...
	function genImageAlarm(a, b) {
		...
		try {
			...
			var c = new Image;                                        //c就是img标签
			c.src = changeColor(graphics, a, b);                      //c.src是一个地址base64
//			c.src='../demo/images/host.png';
			alarmImageCache[a.src] = c;                               //c就是img标签 此处更改警告框                          
			return c.src, alarmImageCache[a.src], c
		} catch(d) {} 
		return null
	}
...
function(a) {
	function b() {
		this.initialize = function() {
			...
			a.rect(-this.width / 2, -this.height / 2, this.width, this.height), a.fill(), a.stroke(), a.closePath()
	...
	function c() {
		this.initialize = function() {
			...
			//设置边框颜色  strokeStyle   设置背景颜色   fillStyle
			0 != this.showSelected && (a.save(), a.beginPath(), a.strokeStyle = "rgba(0,0,0, 0.1)",a.fillStyle = "rgba(255,255,255,0.1)", a.rect(-this.width / 2 - 3, -this.height / 2 - 3, this.width + 6, this.height + 6), a.fill(), a.stroke(), a.closePath(), a.restore())
			...
			if(0 != this.editAble) {
				...
				for(var c = 0; c < b.length; c++) {
					...
					var e = d.right - d.left,
						f = d.bottom - d.top;
					a.beginPath(), a.strokeStyle = "rgba(0,0,0,0.8)", a.rect(d.left, d.top, e, f), a.stroke(), a.closePath(), 
					a.beginPath(), a.strokeStyle = "rgba(255,255,255,0.3)", a.rect(d.left + 1, d.top + 1, e - 2, f - 2), a.stroke(), a.closePath()
				}
				a.restore()
			}
...
function(a) {
	function b(c) {
		this.initialize = function(c) {
			...
			if(this.image) {
				var b = a.globalAlpha;
				a.globalAlpha = this.alpha, null != this.image.alarm && null != this.alarm ? a.drawImage(this.image.alarm,
					-this.width / 2,
					-this.height / 2, this.width, this.height) : a.drawImage(this.image, 
						-this.width / 2, -this.height / 2, this.width, this.height), a.globalAlpha = b
			...
			if(null != this.alarm && "" != this.alarm) {
				...
				var b = this.alarmColor || "255,0,0",
					c = this.alarmAlpha || .5,
					//警告字体颜色参数
					h = this.alarmFontColor || "255,255,255",
					i  = this.alarmFontAlpha || .5;
				a.beginPath(), a.font = this.alarmFont || "10px 微软雅黑";
				var //修改警告位置,
					textArray = this.alarm.split('\n'),
					rowCnt = textArray.length,
					i = 0,imax  = rowCnt,maxLength = 0;
					maxText = textArray[0];
				for(;i<imax;i++){
				...
				var maxWidth = a.measureText(maxText).width;
					lineHeight = a.measureText("田").width,
					d =((a.measureText(this.alarm).width/rowCnt +6)>maxWidth?(a.measureText(this.alarm).width/rowCnt +6):maxWidth),
					e = a.measureText("田").width ,
					//g,g可以修改警告条的宽高
					f = this.width / 2 - d / 2,
					g = (-this.height / 2 - e*rowCnt );
					e=(e)*rowCnt;
